...
* Amazon FSx for ONTAP 및 AWS 자격 증명을 BlueXP 재해 복구 관리에 사용할 BlueXP 작업 공간 내의 작업 환경에 추가해야 합니다.
...
BlueXP에서 아직 계정이 없는 경우 계정을 추가해야 합니다. 을 https://docs.netapp.com/us-en/cloud-manager-setup-admin/concept-netapp-accounts.html["계정 추가 방법에 대한 BlueXP 문서"^]참조하십시오.
...
* BlueXP 재해 복구를 통해 관리할 모든 ONTAP 어레이를 BlueXP 재해 복구 관리에 사용하는 BlueXP 작업 환경에 추가해야 합니다.
+
