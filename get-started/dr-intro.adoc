...
BlueXP 재해 복구는 재해 복구 워크플로우를 자동화하는 클라우드 기반 재해 복구 서비스입니다. 처음에 BlueXP 재해 복구 서비스를 사용하면 온프레미스, NFS 기반 워크로드를 보호하고 기술 미리보기에서 NetApp 스토리지를 실행하는 VMware vSphere VMFS(Virtual Machine File System) 데이터 저장소를 다음 중 하나로 보호할 수 있습니다.
